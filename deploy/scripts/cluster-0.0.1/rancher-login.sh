...
RANCHER_SERVER_URL="https://localhost:9443"
...
rancherForceLogin() {
    ...
    LOGINTOKEN=$(curl -k -s 'https://127.0.0.1:9443/v3-public/localProviders/local?action=login' -H 'content-type: application/json' --data-binary '{"username":"admin","password":"'${DEFAULT_CLUSTER_PASSWORD}'","ttl":60000}' | jq -r .token)
    echo "Login Token: $LOGINTOKEN"
    if [ "$LOGINTOKEN" = null ]; then
        echo "Token is null. trying to set it"
        LOGINTOKEN=$(curl -k -s 'https://127.0.0.1:9443/v3-public/localProviders/local?action=login' -H 'content-type: application/json' --data-binary '{"username":"admin","password":"admin","ttl":60000}' | jq -r .token)
        curl -k -s 'https://127.0.0.1:9443/v3/users?action=changepassword' -H 'Content-Type: application/json' -H "Authorization: Bearer $LOGINTOKEN" --data-binary '{"currentPassword":"admin","newPassword":"'${DEFAULT_CLUSTER_PASSWORD}'"}'
    else
        curl -k -s 'https://127.0.0.1:9443/v3/users?action=changepassword' -H 'Content-Type: application/json' -H "Authorization: Bearer $LOGINTOKEN" --data-binary '{"currentPassword":"'${DEFAULT_CLUSTER_PASSWORD}'","newPassword":"'${DEFAULT_CLUSTER_PASSWORD}'"}'
    fi
    if [ "$LOGINTOKEN" = null ]; then
