...
const HotelAvailable = ({
  ...
  const [selectedRoom, setSelectedRoom] = useState<{ room: string, price: number }[]>([{
    ...
    room: `2 ${t("hotelDetail.hotelAvailable.rooms")}`,
    price: 10
  }, {
    room: `3 ${t("hotelDetail.hotelAvailable.rooms")}`,
    price: 10
  }, {
    room: `4 ${t("hotelDetail.hotelAvailable.rooms")}`,
    price: 10
  }, { room: `5 ${t("hotelDetail.hotelAvailable.rooms")}`, price: 10 }]);
              ...
              <DivView onClick={handleShowCheckin}>
                <StayingInfoWrapper onClick={handleShowCheckin}>
                  ...
                  <CheckBg>
                    ...
                    <p>
                      <strong
                        data-testid="avail-checkInDateInput">{moment(data.checkin).format("DD MMM , ddd")}</strong>
                    </p>
                  </CheckBg>
                </StayingInfoWrapper>
                {show === "checkin" ? (
              ...
              <DivView>
                <StayingInfoWrapper onClick={handleShowCheckout}>
                  ...
                  <CheckBg>
                    ...
                    <p>
                      <strong
                        data-testid="avail-checkOutDateInput">{moment(data.checkout).format("DD MMM , ddd")}</strong>
                    </p>
                  </CheckBg>
                </StayingInfoWrapper>
                {show === "checkout" ? (
          ...
          forArray?.map((w, index) => {
            ...
            return (
              <TableBody key={index}>
                  ...
                  <ColumnThree onClick={() => handleShowRoomList(w.name)} data-testid={`numOfRoomsCont${index}`}>
                    ...
                    <span>
                                            <input type="text" placeholder="1 Rooms" readOnly
                                                   ...
                                                   data-testid={`numOfRoomsInput${index}`} />
                                        </span>
                    <TopUp />
                  </ColumnThree>
                ...
                <ColumnFour
                  data-testid={`amountOfRow${index}`}>{Number(selectedRoom[index].price) * Number(roomUnitPrice)}{" \u20AC"}</ColumnFour>
                <ColumnBreak />
                <ColumnFive>
