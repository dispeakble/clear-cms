...
describe("Hotel Page Suite", () => {
    ...
    it("Should change hotel search input value", async () => {
        const hotelPage = render (<Wrapper {...hotelPageProps} />)

        fireEvent.change(
            hotelPage.getByTestId(/hotelInput/),
            {
                target: {
                    value: "Mohali"
                }
    ...
    it('Should match (In Available Section) adult number with chosen child number', async () => {
        ...
        fireEvent.click(
            hotelPage.getByTestId(/test-overlay-close/)
        )

        await waitFor(() => {
            expect( hotelPage.queryByTestId(/test-overlay-close/)).toBeNull()
        })
    })
