...
function SearchComponent() {
  ...
  const handleAdults = (e: any) => {
    switch (e.target.id) {
      case "adults-minus" :
        if (guests.adults > 1) setGuests(prev => ({
          adults: prev.adults - 1,
          children: prev.children,
          infants: prev.infants
        }));
        break;
      case "adults-plus":
        if (guestsCount() < 10) setGuests(prev => ({
          adults: prev.adults + 1,
          children: prev.children,
          infants: prev.infants
        }));
  ...
  const handleChildren = (e: any) => {
    switch (e.target.id) {
      case "children-minus" :
        if (guests.children > 0) setGuests(prev => ({
          adults: prev.adults,
  ...
  const handleInfants = (e: any) => {
    switch (e.target.id) {
      case "infants-minus" :
        if (guests.infants > 0) setGuests(prev => ({
          adults: prev.adults,
          children: prev.children,
  ...
  return (
    <SearchContainer onSubmit={searchSubmitHandler} data-testid="search-form">
      ...
      <SearchItem>
        ...
        <SearchInput value={destination} onChange={handleDestination} ref={destinationRef} type="text"
