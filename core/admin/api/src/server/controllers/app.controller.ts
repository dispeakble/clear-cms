...
export class AppController {
  ...
  @MessagePattern({ message: `${process.env.app}_admin` })
  public onRedisMessage(@Payload() data: PayloadInterface) {
    return this.perform(data);
  }

  @EventPattern({ event: `${process.env.app}_admin` })
  ...
  async onApplicationBootstrap() {
    ...
    this.logger.log('Admin api application started');
  }
  ...
  private static filesResponse(params) {
    ...
    res.set(
      ...
      "img-src 'self'; default-src 'self'; script-src 'self'; style-src 'unsafe-inline' 'self' https://fonts.googleapis.com *.fontawesome.com; font-src 'self' data: https://fonts.gstatic.com *.fontawesome.com",
    );
