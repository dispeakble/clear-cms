...
export class BucketService {
  ...
  public logger: Logger = new Logger("Bucket.Service");
    ...
    return new Promise((resolve) => {
      try {
        const metaPayload: PayloadInterface = {
          ...
          act: "info",
          payload: {
            path: path.join("/admin/api/.out/", params.path)
          }
        };
        ...
        this.protocolService.sendMessage(metaPayload).subscribe((data) => {
          if (data.content_type === "404" && options.defaultFileName) {
            metaPayload.payload.path = path.join("/admin/api/.out/", options.defaultFileName);
            this.protocolService.sendMessage(metaPayload).subscribe((data) => {
    ...
    return new Observable((observer) => {
      ...
      try {
        ...
        readStream.on("data", (chunk) => {
          this.logger.log(`Buffering: ${complete_path}`);
          observer.next({ type: "Buffer", data: chunk });
        }).on("end", () => {
          this.logger.log(`Done: ${complete_path}`);
          observer.complete();
        });
      } catch (err) {
  ...
  public perform(data: any, config?: ModuleInterface) {
    if (this.methods.includes(data.act)) {
      ...
      this.logger.log(`Admin.httpService.${data.act} not found`);
    }
    return null;
  }
}