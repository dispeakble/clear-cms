...
describe("Hotels checkout page suite", () => {
  it("Should render hotel checkout page", async () => {
  ...
  it("Should go to second step", async () => {
    ...
    await waitFor(() => {
      expect(checkoutPage.getByTestId(/test-hotel-second-step/)).toBeInTheDocument();
    });
  });
  ...
  it("Should go back to first step", async () => {
    ...
    await waitFor(() => {
      expect(checkoutPage.getByTestId(/test-hotel-first-step/)).toBeInTheDocument();
    });
  });
  ...
  it("Should fill inputs and go to third step", async () => {
    ...
    await waitFor(() => {
      expect(checkoutPage.queryByTestId(/test-hotel-second-step/)).not.toBeInTheDocument();
    });

    fireEvent.click(checkoutPage.getByTestId(/test-next-button/));

    await waitFor(() => {
      expect(checkoutPage.queryByTestId(/test-hotel-fourth-step/)).toBeInTheDocument();
    });

    fireEvent.click(checkoutPage.getByTestId(/test-next-button/));

    await waitFor(() => {
      expect(checkoutPage.getByTestId(/test-hotel-checkout-confirmation/)).toBeInTheDocument();
    });
